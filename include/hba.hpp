/**
 * @file hba.hpp
 * @brief 分层 Bundle Adjustment 主框架：LAYER 金字塔层管理、HBA 调度、GTSAM 位姿图优化
 */
#ifndef HBA_HPP
#define HBA_HPP
...
/**
 * @brief HBA 金字塔中的单层数据结构
 *
 * 每一层包含该层的位姿序列、点云、Hessian 信息以及窗口划分参数。
 * Layer 1 从原始 PCD + pose.json 读取；更高层由下层 BA 输出的关键帧点云构成。
 */
class LAYER
{
public:
  ...
  vector<mypcl::pose> pose_vec;                          // 该层位姿序列
  std::vector<thread*> mthreads;                         // 并行 BA 工作线程
  std::vector<double> mem_costs;                         // 各线程内存峰值 (KB)

  std::vector<VEC(6)> hessians;                          // 窗口内位姿对的 Hessian 对角近似，供 PGO 使用
  std::vector<pcl::PointCloud<PointType>::Ptr> pcds;     // 该层点云（Layer1 从磁盘读，其余层由下层生成）
  ...
  /** @brief 预分配线程、点云容器和 Hessian 存储空间 */
  ...
  /**
   * @brief 根据 pose_size 计算滑动窗口划分参数
   *
   * 窗口大小 WIN_SIZE=10，步长 GAP=5：
   *   窗口起点: 0, 5, 10, ...
   *   gap_num = 完整窗口数量 - 1
   *   tail = 末尾不足一整窗的帧数余量
   *   part_length = 每个线程负责的窗口数（动态调整 thread_num 保证负载均衡）
   */
...
/**
 * @brief Hierarchical Bundle Adjustment 主类
 *
 * 流程：
 *   1. 构建 total_layer_num 层金字塔
 *   2. 逐层并行窗口 BA，合并关键帧点云
 *   3. 最后一层 global BA
 *   4. GTSAM ISAM2 位姿图优化，融合各层 Hessian 约束
 */
class HBA
{
public:
  ...
  /**
   * @brief 将当前层 BA 优化后的窗口起点位姿传递给下一层
   * 下一层每个关键帧对应本层某个窗口的第一帧 (index = window_start * GAP)
   */
   ...
   *
   * 构建 GTSAM 因子图，融合两层约束：
   *   - 底层 (Layer 0)：滑动窗口 BA 产生的局部 BetweenFactor
   *   - 顶层 (最后一层)：全局 BA 产生的长程 BetweenFactor
   * Hessian 对角元倒数作为噪声方差，反映约束置信度
   */
