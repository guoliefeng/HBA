/**
 * @file tools.hpp
 * @brief HBA 基础工具：李代数运算、体素索引、点云降采样、增量协方差统计
 */
#ifndef TOOLS_HPP
#define TOOLS_HPP
...
// 由向量 v 构造反对称矩阵的宏（用于 Rodrigues 公式）
...
#define DIMU 18           // IMU 状态增量维度
#define DIM 15            // 状态向量维度
#define DNOI 12           // 噪声维度
#define NMATCH 5          // 平面拟合最少点数
#define DVEL 6            // 位姿增量维度 (旋转3 + 平移3)
...
/** @brief 体素网格整数坐标，用作 unordered_map 的键 */
class VOXEL_LOC
{
public:
  int64_t x, y, z;
...
namespace std
{
  /** @brief VOXEL_LOC 的哈希函数，用于体素地图快速索引 */
  template<>
...
/** @brief SO(3) 指数映射：旋转向量 -> 旋转矩阵 (Rodrigues) */
...
/**
 * @brief IMU/位姿状态结构体
 * BA 优化中主要使用 R(旋转) 和 p(平移)，其余字段保留供扩展
 */
struct IMUST
{
  double t;              // 时间戳
  Eigen::Matrix3d R;     // 旋转矩阵
  Eigen::Vector3d p;     // 平移向量
  Eigen::Vector3d v;     // 速度
  ...
  /** @brief 流形上的加法：旋转左乘 Exp(δθ)，其余分量直接相加 */
...
/**
 * @brief 体素降采样，降采样后的点坐标为体素内点的质心
 * @param pc 输入输出点云（原地替换）
 * @param voxel_size 体素边长，小于 0.01 时不降采样
 */
...
/** @brief 将原始点向量 porig 变换到 ptran：ptran[i] = R*porig[i] + p */
...
/**
 * @brief 体素内点云的增量统计量
 * 维护 N(点数)、v(坐标和)、P(坐标外积和)，可高效合并与协方差变换
 * 协方差: cov = P/N - (v/N)(v/N)^T
 */
class VOX_FACTOR
{
public:
...
/**
 * @brief 用 NMATCH 个点拟合平面 ax+by+cz+1=0
 * @return 拟合成功且内点误差小于 threshold 时返回 true
 */
