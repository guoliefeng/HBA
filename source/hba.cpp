/**
 * @file hba.cpp
 * @brief HBA 主程序：分层并行窗口 BA、全局 BA、线程调度
 *
 * 核心流程：
 *   cut_voxel -> recut -> tras_opt -> remove_outlier -> damping_iter -> 合并关键帧
 */
#include <iostream>
#include <iomanip>
#include <fstream>
...
/** @brief PCD 文件名零填充位数，由 launch 参数传入 */
int pcd_name_fill_num = 0;

/**
 * @brief 将一帧点云按体素坐标插入地图哈希表
 *
 * 每个体素对应一棵八叉树根节点 OCTO_TREE_ROOT，存储：
 *   - vec_orig / vec_tran：雷达系 / 世界系下的点
 *   - sig_orig / sig_tran：对应的 VOX_FACTOR 增量统计
 *
 * @param feat_map   体素地图 (VOXEL_LOC -> 八叉树根)
 * @param feat_pt    当前帧点云（雷达系）
 * @param q, t       当前帧位姿
 * @param fnum       当前帧在滑窗内的索引 [0, window_size)
 * @param voxel_size 体素边长
 * @param window_size 滑窗大小
 * @param eigen_ratio 平面判定阈值（越大越宽松）
 */
...
/**
 * @brief 并行 BA 工作函数（前 thread_num-1 个线程）
 *
 * 每个线程处理 part_length 个完整滑动窗口。
 * 每个窗口：迭代 BA -> 合并点云为关键帧 -> 写入 next_layer.pcds
 *
 * @param layer      当前层
 * @param thread_id  线程编号
 * @param next_layer 下一层（输出关键帧点云）
 */
void parallel_comp(LAYER& layer, int thread_id, LAYER& next_layer)
{
  ...
  for(int i = thread_id*part_length; i < (thread_id+1)*part_length; i++)
  {
    ...
    {
      // Layer 1 时从磁盘加载 PCD（仅 loop==0 读盘，后续复用 raw_pc）
      if(layer_num == 1)
        for(int j = i*GAP; j < i*GAP+WIN_SIZE; j++)
        {
          if(loop == 0)
...
/**
 * @brief 并行 BA 尾线程工作函数（最后一个线程）
 *
 * 处理 left_gap_num 个完整窗口 + 可能的 tail 不完整窗口。
 * 相比 parallel_comp 增加了各阶段耗时统计。
 */
...
/**
 * @brief 最后一层全局 BA：对所有关键帧联合优化
 *
 * 与窗口 BA 流程相同，但 window_size = 全部帧数，
 * eigen_ratio 放宽为 2 倍以加速收敛。
 */
void global_ba(LAYER& layer)
{
  ...
  for(int loop = 0; loop < layer.max_iter; loop++)
  {
    ...
    for(int i = 0; i < window_size; i++)
    {
      ...
      dsp_t += ros::Time::now().toSec() - t0;
      t0 = ros::Time::now().toSec();
      // 全局 BA 使用更宽松的平面阈值 (2 * eigen_ratio)
...
/**
 * @brief 启动多线程并行 BA 并等待完成
 *
 * 前 thread_num-1 个线程执行 parallel_comp，最后一个执行 parallel_tail。
 */
 ...
 * @brief HBA 主入口
 *
 * 1. 逐层并行窗口 BA (total_layer_num - 1 次)
 * 2. 最后一层 global BA
 * 3. GTSAM 位姿图优化
 */
